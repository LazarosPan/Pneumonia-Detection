...
transform = transforms.Compose([
    ...
    transforms.RandomResizedCrop(224, scale=(0.8, 1.0)),  # Increase the crop range
    transforms.ToDtype(torch.float32, scale=True),  # Replace ToTensor with the recommended transforms
...
dataloader = DataLoader(dataset, batch_size=60, shuffle=True, num_workers=num_cpu_cores, pin_memory=True, persistent_workers=True)
...
class PneumoniaModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = Adam(self.parameters(), lr=1e-4)  # Use Adam optimizer with specified learning rate
        scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=5)  # Reduce learning rate on plateau
...
early_stopping_callback = EarlyStopping(monitor='train_loss', patience=13, mode='min')
...
if __name__ == '__main__':
    ...
    trainer = pl.Trainer(max_epochs=100, accelerator='cuda', callbacks=[progress_bar, early_stopping_callback, lr_monitor]) if torch.cuda.is_available() else pl.Trainer(max_epochs=10, callbacks=[progress_bar, early_stopping_callback, lr_monitor])
    ...
    results_df.to_csv(location + 'results/v3_resnet50_gpu_s50k_b60_e100.csv', index=False)

    # Specify the directory and file name to save the model
    model_save_path = location + "models/v3_resnet50_gpu.pth"
